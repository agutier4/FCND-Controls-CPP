...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
   ...
   if ( collThrustCmd > 0 )
   {
     float targetR13 = -CONSTRAIN( accelCmd.x / c_d, -maxTiltAngle, maxTiltAngle );
     float targetR23 = -CONSTRAIN( accelCmd.y / c_d, -maxTiltAngle, maxTiltAngle );
    
     float rollErr = R(0,2) - targetR13;
     float pitchErr = R(1,2) - targetR23;

     pqrCmd.x = 1/R(2,2) * ( kpBank * (-R(1,0)*rollErr + R(0,0)*pitchErr) );
     pqrCmd.y = 1/R(2,2) * ( kpBank * (-R(1,1)*rollErr + R(0,1)*pitchErr) );
...
float QuadControl::AltitudeControl(float posZCmd, float velZCmd, float posZ, float velZ, Quaternion<float> attitude, float accelZCmd, float dt)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  float zPosErr = posZCmd - posZ;
  float zDotCmd = (kpPosZ * zPosErr) + velZCmd;
  integratedAltitudeError += zPosErr*dt; 

  zDotCmd = CONSTRAIN( zDotCmd, -maxDescentRate, maxAscentRate );

  float zDotDot = kpVelZ * (zDotCmd - velZ) + accelZCmd + (KiPosZ * integratedAltitudeError);
  thrust = -((zDotDot-9.81) * mass) / R(2,2);
  /////////////////////////////// END STUDENT CODE ////////////////////////////
